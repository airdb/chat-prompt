...
func (ba *BaseApi) Captcha(c *gin.Context) {
	// 字符,公式,验证码配置
	// 生成默认数字的driver
	driver := base64Captcha.NewDriverDigit(global.CHPT_CONFIG.Captcha.ImgHeight, global.CHPT_CONFIG.Captcha.ImgWidth, global.CHPT_CONFIG.Captcha.KeyLong, 0.7, 80)
	// cp := base64Captcha.NewCaptcha(driver, store.UseWithCtx(c))   // v8下使用redis
	cp := base64Captcha.NewCaptcha(driver, store)
	id, b64s, err := cp.Generate()
	if err != nil {
		global.CHPT_LOG.Error("验证码获取失败!", zap.Error(err))
	...
	response.OkWithDetailed(systemRes.SysCaptchaResponse{
		CaptchaId:     id,
		PicPath:       b64s,
		CaptchaLength: global.CHPT_CONFIG.Captcha.KeyLong,
	}, "验证码获取成功", c)
}
...
func (ba *BaseApi) Login(c *gin.Context) {
	...
	if err := validate.Struct(&login); err != nil {
		...
		global.CHPT_LOG.Error("请求参数错误", zap.Error(err))
		return
	}
	...
	if store.Verify(login.CaptchaId, login.Captcha, true) {
		...
		if err != nil {
			global.CHPT_LOG.Error("登陆失败", zap.Error(err))
...
func tokenNext(c *gin.Context, user *system.UserModel) {
	j := &utils.JWT{SigningKey: []byte(global.CHPT_CONFIG.JWT.SigningKey)} // 唯一签名

	claims := systemReq.CustomClaims{
		ID:         user.ID,
		Username:   user.Username,
		RoleId:     user.RoleModelID,
		BufferTime: global.CHPT_CONFIG.JWT.BufferTime,
		RegisteredClaims: jwt.RegisteredClaims{
			NotBefore: jwt.NewNumericDate(time.Now().Add(-time.Duration(1000))),
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(time.Duration(global.CHPT_CONFIG.JWT.ExpiresTime) * time.Second)),
			Issuer:    global.CHPT_CONFIG.JWT.Issuer,
		},
	}
	...
	if err != nil {
		...
		global.CHPT_LOG.Error("获取token失败", zap.Error(err))
		return
	}
	...
	if jwtStr, err := jwtService.GetRedisJWT(user.Username); err == redis.Nil {
		if err := jwtService.SetRedisJWT(user.Username, token); err != nil {
			...
			global.CHPT_LOG.Error("设置登录状态失败", zap.Error(err))
			return
		}
