...
func (ms *MenuService) GetMenus(userId uint) ([]systemModel.MenuModel, error) {
	...
	err := global.CHPT_DB.Where("id = ?", userId).First(&userModel).Error
	if err != nil {
	...
	err = global.CHPT_DB.Where("id = ?", userModel.RoleModelID).First(&roleModel).Error
	if err != nil {
		return nil, fmt.Errorf("GetMenus 角色查询 -> %v", err)
	}

	var menuModels []systemModel.MenuModel
	err = global.CHPT_DB.Preload("Roles").Find(&menuModels).Error
	if err != nil {
...
func (ms *MenuService) AddMenu(menuRaw systemReq.Menu) bool {
	...
	if err := global.CHPT_DB.Create(&menuModel).Error; err != nil {
		global.CHPT_LOG.Error("创建menu失败", zap.Error(err))
		return false
	}
...
func (ms *MenuService) EditMenu(menuRaw systemReq.EditMenuReq) (err error) {
	...
	if errors.Is(global.CHPT_DB.Where("id = ?", menuRaw.Id).First(&menuModel).Error, gorm.ErrRecordNotFound) {
	...
	err = global.CHPT_DB.Model(&menuModel).Updates(map[string]interface{}{
		"pid":       menuRaw.Pid,
		"name":      menuRaw.Name,
		"path":      menuRaw.Path,
...
func (ms *MenuService) DeleteMenu(id uint) (err error) {
	var menuModel systemModel.MenuModel
	if errors.Is(global.CHPT_DB.Where("id = ?", id).First(&menuModel).Error, gorm.ErrRecordNotFound) {
		return errors.New("菜单不存在")
	}
	err = global.CHPT_DB.Unscoped().Select("Roles").Delete(&menuModel).Error
...
func (ms *MenuService) GetElTreeMenus(roleId uint) ([]systemModel.MenuModel, []uint, error) {
	var menuModels []systemModel.MenuModel
	err := global.CHPT_DB.Find(&menuModels).Error
	if err != nil {
		global.CHPT_LOG.Error("GetElTreeMenus 查询menus", zap.Error(err))
		return nil, nil, err
	}
	...
	var roleModel systemModel.RoleModel
	err = global.CHPT_DB.Where("id = ?", roleId).Preload("Menus").First(&roleModel).Error
	if err != nil {
		global.CHPT_LOG.Error("GetElTreeMenus 查询role", zap.Error(err))
		return nil, nil, err
	}
