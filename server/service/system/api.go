...
func (a *ApiService) AddApi(api systemModel.ApiModel) (*systemModel.ApiModel, error) {
	if !errors.Is(global.CHPT_DB.Where("path = ? AND method = ?", api.Path, api.Method).First(&systemModel.ApiModel{}).Error, gorm.ErrRecordNotFound) {
		return nil, errors.New("存在相同api")
	}

	err := global.CHPT_DB.Create(&api).Error
...
func (a *ApiService) GetApis(apiSp systemReq.ApiSearchParams) ([]systemModel.ApiModel, int64, error) {
	...
	db := global.CHPT_DB.Model(&systemModel.ApiModel{})
...
func (a *ApiService) GetElTreeApis(roleId uint) (list []systemModel.ApiTree, checkedKey []string, err error) {
	...
	err = global.CHPT_DB.Find(&apiModels).Error
	if err != nil {
		return nil, nil, fmt.Errorf("GetElTreeApis: find -> %v", err)
	}

	var apiGroup []string
	err = global.CHPT_DB.Model(&systemModel.ApiModel{}).Distinct().Pluck("api_group", &apiGroup).Error
	if err != nil {
...
func (a *ApiService) DeleteApi(id uint) (err error) {
	...
	err = global.CHPT_DB.Where("id = ?", id).First(&apiModel).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		global.CHPT_LOG.Error("deleteApi -> 查找id", zap.Error(err))
		return err
	}

	err = global.CHPT_DB.Unscoped().Delete(&apiModel).Error
	if err != nil {
		global.CHPT_LOG.Error("deleteApi -> 删除id", zap.Error(err))
		return err
	}
...
func (a *ApiService) EditApi(eApi systemReq.EditApi) (err error) {
	...
	err = global.CHPT_DB.Where("id = ?", eApi.Id).First(&oldApiModel).Error
	if err != nil {
		return errors.New("editApi: id不存在")
	}

	if oldApiModel.Path != eApi.Path || oldApiModel.Method != eApi.Method {
		if !errors.Is(global.CHPT_DB.Where("path = ? AND method = ?", eApi.Path, eApi.Method).First(&systemModel.ApiModel{}).Error, gorm.ErrRecordNotFound) {
	...
	return global.CHPT_DB.Debug().Model(&oldApiModel).Updates(map[string]interface{}{"path": eApi.Path, "method": eApi.Method, "api_group": eApi.ApiGroup, "description": eApi.Description}).Error
}
